...
const ComponentCard = ({ 
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'preview' && (
          <motion.div 
            ...
            className="relative"
          >
            <div className="p-8 bg-gray-900/50 rounded-lg flex items-center justify-center min-h-[200px]">
              {component}
            </div>
          </motion.div>
        )}
        ...
        {activeTab === 'jsx' && (
          ...
          >
            <button
              className={`absolute top-3 right-3 px-3 py-1.5 text-xs rounded-md z-10 flex items-center gap-1 border ${
  ...
  return (
    <div className="w-full bg-gray-900/30 backdrop-blur-sm border border-gray-800 rounded-xl overflow-hidden shadow-lg mb-8">
      {/* Header */}
      ...
      <div className="flex border-b border-gray-800 relative">
        {["preview", "jsx", "js"].map((tab) => (
          <motion.button
            key={tab}
            className={`px-5 py-3 text-sm font-medium relative ${activeTab === tab ? 'text-purple-400' : 'text-gray-400'}`}
